...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	protected void addSingletonFactory(String beanName, ObjectFactory singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			if (!this.singletonObjects.containsKey(beanName)) {
				// 将bean 的 ObjectFactory 添加进三级缓存中
	...
	public Object getSingleton(String beanName, ObjectFactory singletonFactory) {
		Assert.notNull(beanName, "'beanName' must not be null");
		synchronized (this.singletonObjects) {
			// 首先检查对应的bean是否已经加载过，因为singletBean模式其实就是复用已经创建的bean，所以这一步是必须的
			Object singletonObject = this.singletonObjects.get(beanName);
			// 条件成立则说明此bean未创建过，否则直接return
			if (singletonObject == null) {
				...
				// 创建bean前，将当前创建的bean记录在缓存中，表示当前bean处于加载中状态。
				// 这样便可以对循环依赖进行检测
				...
				try {
					/**
					 * 这里使用工厂模式来获取单例bean
					 * 注意这里调用的外层createBean(beanName, mbd, args);创建bean
					 */
					singletonObject = singletonFactory.getObject();
				}
					...
					// 到这里说明bean已经创建完毕，则移除掉bean在缓存中正在加载的状态，对应前面的before
